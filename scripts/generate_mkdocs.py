...
- Map folder/00_artifact.md to folder nav entry
...
def build_nav_tree(start_path: Path, root_docs: Path) -> Optional[Dict]:
    ...
    for entry in entries:
        ...
        if entry.is_dir():
            ...
            # then include other children (recursively). We no longer treat 00_artifact.md as folder index.
            index_file = None
            ...
            if EXCLUDE_PATTERN.search(entry.name):
                continue
            # Skip 00_artifact.md (will be handled by parent as folder index)
...
def generate_mkdocs_config() -> Dict:
    ...
    for entry in nav_list:
        ...
        for v in entry.values():
            if isinstance(v, str):
                # Check only the basename so paths like '2022-2025/00_artifact.md' are handled
                if os.path.basename(v).lower() == 'index.md':
                    has_root_index = True
    if not has_root_index:
        # find first top-level index under docs/ (exclude 00_artifact.md)
        first_index = None
